...
SELECT *
FROM Customers
WHERE Country LIKE '%a' OR Country LIKE '%A';
Using Products table, show the highest price among all products.
SELECT MAX(Price) AS HighestProductPrice
FROM Products;
...
SELECT MIN(Quantity) AS MinimumQuantityOrdered, MAX(Quantity) AS MaximumQuantityOrdered
FROM Orders;
Medium-Level Tasks
...
AND o.OrderDate >= '2023-01-01' AND o.OrderDate < '2023-02-01'
...
-- Inserting pivoted data (SQL Server syntax)
...
-- Inserting pivoted data (SQL Server syntax)
INSERT INTO Population_Each_City (district_id, year, Bektemir, Chilonzor, Yakkasaroy)
...
Here are the SQL queries for your medium and hard-level tasks:

Medium-Level Tasks
List customer IDs who placed orders in January 2023 but did not have invoices
SQL
...
SELECT ProductName
FROM Products_Discounted;
Find the average order amount by year
SQL
...
SELECT
    ...
    END AS PriceGroup
FROM Products;
Use Pivot to show values of Year column in separate columns ([2012], [2013]) and copy results to a new Population_Each_Year table.
(Note: SQL Server's PIVOT syntax is shown below. Syntax may vary slightly for other database systems like MySQL or PostgreSQL.)
...
Find total sales per product Id
SQL

SELECT ProductID, SUM(SaleAmount) AS TotalSales
FROM Sales
GROUP BY ProductID;
Use wildcard to find products that contain 'oo' in the name
SQL

SELECT ProductName
FROM Products
WHERE ProductName LIKE '%oo%';
Use Pivot to show values of City column in separate columns (Bektemir, Chilonzor, Yakkasaroy) and copy results to a new Population_Each_City table.
(Note: SQL Server's PIVOT syntax is shown below. Syntax may vary slightly for other database systems like MySQL or PostgreSQL.)
...
Hard-Level Tasks
Show top 3 customers with the highest total invoice amount
SQL
...
GROUP BY CustomerID
ORDER BY TotalSpent DESC
LIMIT 3; -- Use TOP 3 for SQL Server, LIMIT 3 for MySQL/PostgreSQL
Transform Population_Each_Year table to its original format (City_Population)
SQL
	...
	SELECT district_id, district_name, "2013" AS population, '2013' AS year
	FROM Population_Each_Year;
List product names and the number of times each has been sold
SQL

SELECT p.ProductName, COUNT(s.SaleID) AS NumberOfTimesSold
FROM Products p
JOIN Sales s ON p.ProductID = s.ProductID
GROUP BY p.ProductName
ORDER BY NumberOfTimesSold DESC;
Transform Population_Each_City table to its original format (City_Population).
	SELECT district_id, 'Bektemir' AS district_name, Bektemir AS population, year
	FROM Population_Each_City
	WHERE Bektemir IS NOT NULL
	UNION ALL
	SELECT district_id, 'Chilonzor' AS district_name, Chilonzor AS population, year
	FROM Population_Each_City
	WHERE Chilonzor IS NOT NULL
	UNION ALL
	SELECT district_id, 'Yakkasaroy' AS district_name, Yakkasaroy AS population, year
	FROM Population_Each_City
	WHERE Yakkasaroy IS NOT NULL;
